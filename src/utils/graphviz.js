...
export function generateDOT(
  module: Module,
  selectedState: State,
  selectedStateTransition: Number
) {
  let relatedStates = {};
  ...
  if (selectedState) {
    ...
    relatedStates = Object.assign(
      {},
      relatedStates,
      findTransitions(module.states[selectedState.name])
    );
  }
...
const nodesAsDOT = (
  ...
  return Object.keys(module.states)
    .filter((name) => typeof module.states[name] === "object")
    .map((name) => {
  ...
  return Object.keys(module.states)
    .map((name) => {
      ...
      if (state.direct_transition !== undefined) {
        if (
          selectedState &&
          state.direct_transition === selectedState.name &&
        ...
        if (module.states[state.direct_transition]) {
          return `  "${escapedName}" -> "${escapeName(
            module.states[state.direct_transition].name
          )}" [class = "transition transition-index_0 ${className}"];\n`;
        } else {
          console.log(
            `NO SUCH NODE TO TRANSITION TO: ${state.direct_transition} FROM ${name}`
          );
        }
        ...
        state.distributed_transition.forEach((t, i) => {
          ...
          if (module.states[t.transition]) {
            if (
              selectedState &&
              t.transition === selectedState.name &&
              selectedState.name !== name
            ) {
              transitionClassName = "";
            }
            ...
            out_transitions += `  "${escapedName}" -> "${escapeName(
              module.states[t.transition].name
            )}" [label = "${distLabel}", class = "transition transition-index_${i} ${transitionClassName}"];\n`;
          } else {
            console.log(
              `NO SUCH NODE TO TRANSITION TO: ${t.transition} FROM ${name}`
            );
          }
        });
        ...
        state.lookup_table_transition.transitions.forEach((t, i) => {
          ...
          if (module.states[t.transition]) {
            ...
            out_transitions += `  "${escapedName}" -> "${escapeName(
              module.states[t.transition].name
            )}" [label = "See Table (def: ${distLabel})", class = "transition transition-index_${i} ${transitionClassName}"];\n`;
          } else {
            console.log(
              `NO SUCH NODE TO TRANSITION TO: ${t.transition} FROM ${name}`
            );
        ...
        state.conditional_transition.forEach((t, i) => {
          let transitionClassName = className;
          let cnd =
            t.condition !== undefined ? logicDetails(t.condition) : "else";
          if (module.states[t.transition]) {
            ...
            out_transitions += `  "${escapedName}" -> "${escapeName(
              ...
              i + 1
            }. ${cnd}", class = "transition transition-index_${i} ${transitionClassName}"];\n`;
          } else {
            console.log(
              `NO SUCH NODE TO TRANSITION TO: ${t.transition} FROM ${name}\n`
            );
          ...
          if (t.transition !== undefined) {
            ...
            t.distributions.forEach((dist) => {
              if (module.states[dist.transition]) {
                ...
                console.log(
                  `NO SUCH NODE TO TRANSITION TO: ${dist.transition} FROM ${name}`
                );
              }
            });
          }
...
const stateDescription = (state) => {
  ...
  switch (state["type"]) {
    ...
    case "SetAttribute":
      ...
      let v = state["value"];
      details = `Set '${state["attribute"]}' = ${
        v === undefined || v === null || v === "" ? "nil" : v
      }`;
      break;
    case "Symptom":
      let s = state["symptom"];
    ...
    case "Counter":
      details = `${state["action"]} value of attribute '${
        state["attribute"]
      }' by ${state["amount"] || 1}`;
      break;
    case "VitalSign":
  ...
  if (state.assign_to_attribute) {
    details =
      details + `Assign to Attribute: '${state["assign_to_attribute"]}'\\l`;
  }
  if (state.clinical_status) {
    details = details + `Clinical Status: '${state["clinical_status"]}'\\l`;
  }
  ...
  if (state.referenced_by_attribute) {
    details =
      details +
      `Referenced By Attribute: '${state["referenced_by_attribute"]}'\\l`;
  }
  if (state.activities) {
  ...
  if (state.goals) {
    ...
    state["goals"].forEach((goal) => {
      if (goal["text"]) {
        ...
        details =
          details +
          `Observation ${obs} \\${logic["operator"]}${leaveUndefinedBlank(
            logic["value"]
          )}\\l`;
      }
    });
  ...
  if (state.period) {
    // if(state.duration){
    let d = state["period"];
    details = `${details}\\n Start Date: ${moment(d["lowdate"]).format(
...
const logicDetails = (logic) => {
  switch (logic["condition_type"]) {
    ...
    case "Attribute":
      return `Attribute: '${logic["attribute"]}' \\${
        logic["operator"]
      }${leaveUndefinedBlank(logic["value"])}\\l`;
    case "Observation":
      let obs = findReferencedType(logic);
      if (
        logic.operator !== "is nil" &&
        logic.operator !== "is not nil" &&
        logic.operator.id !== "is nil" &&
        logic.operator.id !== "is not nil"
      ) {
